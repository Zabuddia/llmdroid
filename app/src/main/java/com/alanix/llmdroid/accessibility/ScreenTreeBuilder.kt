...
object ScreenTreeBuilder {
    ...
    private fun walkTree(
        ...
        try {
            // Skip LLMDroid's own UI nodes so the agent never sees them
            if (node.packageName?.toString() == "com.alanix.llmdroid") return

            val rect = Rect()
            node.getBoundsInScreen(rect)

            // Skip elements scrolled off-screen — tapping negative coords always fails
            if (rect.left < 0 || rect.top < 0 || rect.width() <= 0 || rect.height() <= 0) {
                for (i in 0 until node.childCount) {
                    val child = node.getChild(i) ?: continue
                    walkTree(child, elements, depth + 1, parentDesc)
                }
                return
            }
            ...
            if (isInteractive || displayText.isNotEmpty()) {
                ...
                elements.add(
                    UIElement(
                        ...
                        size = listOf(rect.width(), rect.height()),
            ...
            // Node may be recycled during traversal
        }
    }
